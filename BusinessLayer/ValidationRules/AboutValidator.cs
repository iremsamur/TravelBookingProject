...
namespace BusinessLayer.ValidationRules
{
    ...
    {
        //About için validasyonları kuralları yazdığım sınıf
        public AboutValidator()
        {
            RuleFor(x => x.Description).NotEmpty().WithMessage("Açıklama kısımını boş geçemezsiniz...");
            RuleFor(x => x.Image1).NotEmpty().WithMessage("Lütfen görsel seçiniz...!");
            RuleFor(x => x.Description).MinimumLength(50).WithMessage("Lütfen en az" +
                "50 karakterlik açıklama bilgisi giriniz...!");
